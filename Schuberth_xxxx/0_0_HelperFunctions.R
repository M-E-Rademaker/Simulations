...
computeRelevant <- function(.object, .R) {
  ...
  if(inherits(.object, "cSEMResults_2ndorder")) {
    ...
    ## Only compute test if no repeated indicators are present (in this case the 
    ## length of data would be 33)
    if(ncol(.object$Information$Data) == 21) {
      ## Joint test
      out2a <- testOMF(.object, 
                       .handle_inadmissibles = "replace",
                       .alpha = c(0.1, 0.05, 0.01),
...
checkGuidlines <- function(.data, .model, .approach, .what, .scheme = "path") {
  
  # Estimate
  if(.approach == "emb_TS") {
    out <- csem(.data, .model, .approach_2ndorder = "mixed")
  } else {
    out <- csem(.data, .model, .PLS_weight_scheme_inner = .scheme)
  }
